...
class BookingService {
    ...
    async updateBookingStatus(sitterId, bookingId, newStatus) {
    ...
    async sitterCheckIn(bookingId, sitterId, latitude, longitude) {
        const query = `
            UPDATE bookings
            SET status = 'ACTIVE',
                check_in_time = NOW(),
    ...
    async sitterCheckOut(bookingId, sitterId) {
        const query = `
            UPDATE bookings
    ...
    // ----------------------------------------------
    async transitionBookingStatus(sitterId, bookingId, expectedStatus, newStatus, action) {
        const checkQuery = 'SELECT status, sitter_id FROM bookings WHERE booking_id = $1';
        const checkResult = await this.pool.query(checkQuery, [bookingId]);
        if (checkResult.rows.length === 0)
            throw new Error("Reserva no encontrada.");
    ...
    async validateReviewEligibility(bookingId, clientId) {
        const query = `
