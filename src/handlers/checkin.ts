 ...
 */
export async function checkinHandler(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
    ...
    try {
        ...
        const { isWithinArea, distance } = geoService.isWithinServiceArea(
            latitude, 
            longitude, 
            serviceLocation.lat, 
            serviceLocation.lon
